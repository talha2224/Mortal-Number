...
const acceptedCreditRequest = async (requestId, getterId, setterid, amount) => {
  let creditInfo = await GetterCreditModel.findByIdAndUpdate(
    requestId,
    {
      $set: { approved: true },
    },
  ...
  if (!creditInfo) {
    throw new ErrorResponse(
      "wrong credit request id given no such request found",
      404
    );
  } else {
    if (creditInfo.getterProfileId) {
      let getterInfo = await GetterRegisterModel.findById(
        creditInfo.getterProfileId
      );
      let updateGetterCredit = await GetterRegisterModel.findByIdAndUpdate(
        creditInfo.getterProfileId,
        {
          $set: {
...
const getCreditHistory = async (id) => {
  let CreditHistory = await GetterCreditModel.find({
    $or: [{ setterProfileId: id }, { getterProfileId: id }],
  });
  if (CreditHistory) {
...
//ALL REQUESTS
const getAll = async () => {
  const allRequest = await GetterCreditModel.find({ approved: false }).populate(
    "getterProfileId setterProfileId",
    "-OTP -otpValidTill -otpVerified -email -password -phonenumber -dateOfBirth -gender -country -__v"
  );
